...
class Pedigree(HTMLPage):
    ...
    def render(self, targetid, maxlevel = MAX_GENERATIONS) -> str:
        ...
        def render_collaspe_form() -> str:
            ...
            output += "<INPUT TYPE=\"button\" NAME=\"action\" VALUE=\"Collaspe\" onClick=\"javascript:formHandler(this, 0)\" />\n"
            output += "</FORM>\n"
            output += "</CENTER>\n"
            output += "<script>\n"
            output += "var slider = document.getElementById(\"myRange\");\n"
            output += "var output = document.getElementById(\"demo\");\n"
        ...
        def render_script() -> str:
            ...
            output += "   function formHandler(thisItem, isExpand) {\n"
            output += "       var slider = document.getElementById(\"myRange\");\n"
            output += "       traversebranches(" + str(mainperson_line_number) + ", 1, slider.value, isExpand); \n" 
            output += "   }\n"
            output += "   function traversebranches(line_num, level, leveltochange, isExpand) {\n"
            ...
            output += "         }\n"
            output += "         toggle(\"DivID\" + line_num, 1);"
            ...
            output += "               traversebranches(father_line_number, level + 1, leveltochange, isExpand);\n"
            output += "            }\n"
            output += "            var mother_line_number = ancestors_line_number[line_num][1];\n"
            output += "            if (mother_line_number > 0) {\n"
            output += "               traversebranches(mother_line_number, level + 1, leveltochange, isExpand);\n"
            output += "            }\n"
            output += "         }\n"
            output += "      }\n"
            output += "      if (level == leveltochange) {\n"
            output += "         hidebranches(line_num, 1, isExpand);\n"
            output += "         return;\n"
            output += "      }\n"
            output += "   }\n"
            ...
            output += "            toggle(\"DivID\" + line_num + \"e\", 1);\n"
            output += "            toggle(\"DivID\" + line_num + \"f\", 0);\n"
            output += "         } else if (closeorexpand == 0) {\n"
            output += "            toggle(\"DivID\" + line_num, 1);"
            output += "            document.getElementById(\"ButtonID\" + line_num ).innerHTML = \"+\";\n"
