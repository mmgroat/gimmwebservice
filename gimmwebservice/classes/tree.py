...
class Source:
    ...
    def pretty_print(self)->str:
        output = ""
        if self.title:
            output += "Title: " + str(self.title) + "<br>"
        if self.citation:
            output += "Author: " + str(self.citation) + "<br>"
        if self.url:
            output += "Publication: " + str(self.url) + "<br>"
        if self.notes:
            if len(self.notes):
...
class Indi:
    ...
    def pretty_print_birth(self) -> str:
        """Get preferred Birth (1st one) as b. DATE in PLACE depending on DATE/PLACE Existence"""
        output = ""
        for fact in self.facts:  # self.tree.indi[parent].
            # GET BIRTH
            if fact.type == FACT_TYPES["BIRT"]:
                output = "<B>b.</B> "
                if fact.date is not None or fact.place is not None:
                    if fact.date is not None:
                        output += str(fact.date) + " "
                    if fact.place is not None: 
                        output += "in " + str(fact.place) 
                    return output
                else:
                    return ""
        return output
                    
    def pretty_print_death(self) -> str:
        """Get preferred Birth (1st one) as b. DATE in PLACE depending on DATE/PLACE Existence"""
        output = ""
        for fact in self.facts:  # self.tree.indi[parent].
            # GET BIRTH
    ...
    def pretty_print_marriage_preferred(self, tree) -> str:
        output = ""
        family = None
        if self.fams_num_spouses_num:
            husb, wife = list(self.fams_num_spouses_num)[0] # return preferred spouse marriage information
            if (husb, wife) in tree.fam:
                family = tree.fam[(husb, wife)]
                if family:
                    for fact in family.facts: # return preferred marriage information
    ...
    def pretty_print_all_marriage_facts_by_spouseid(self, spouseid, tree) -> set:
        output = set()
        family = None
        if self.fams_num_spouses_num:
            for husb, wife in self.fams_num_spouses_num:
                if husb == spouseid or wife == spouseid:
                    if (husb, wife) in self.tree.fam:
                        family = tree.fam[(husb, wife)]
                        if family:
                            for fact in family.facts: # return preferred marriage information
                                if fact.type == FACT_TYPES["MARR"]:
                                    if fact.date is not None or fact.place is not None:
                                        tempoutput = "<B>m.</B> "
                                        if fact.date is not None:
                                            tempoutput += str(fact.date) + " "
                                        if fact.place is not None:
                                            tempoutput += "in " + str(fact.place)
                                        output.add(tempoutput)
        return output
    
    def get_children_set_by_spouse_id(self, spouseid) -> set:
        output = set()
        for father, mother, child in self.children:
        ...
        if rels:
            ...
            for father, mother, _ in rels:
                if father in self.indi and mother in self.indi:
                    self.indi[father].add_fams((father, mother))
                    self.indi[mother].add_fams((father, mother))
                    # MMG - BUG? since tree.fam is a set, a spouse with multiple unknowns are the same family since they would be the same index?
                    self.add_fam(father, mother)
